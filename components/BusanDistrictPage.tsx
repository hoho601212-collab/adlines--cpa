...
export default function BusanDistrictPage({district:rawDistrict}:{district:BusanDistrict}){
 const district=withBusanDistrictEvidence(rawDistrict), keyword=`${district.name} 태아보험`;
 const siblings=busanDistricts.filter(d=>d.slug!==district.slug), evidence=getBusanEvidenceState(district), seoIntent=getBusanSeoIntent(district), flow=getBusanPageFlow(district);
 const focusNotes=getBusanFocusNotes(district), supportNotes=getBusanSupportNotes(district), topicFaq=getBusanTopicFaq(district);
 const faq=[topicFaq,{q:`${district.name}에 살면 태아보험 보장내용이 달라지나요?`,a:'보험상품의 보장과 심사기준은 거주 구·군 때문에 달라지는 것이 아닙니다. 지역 구분은 부산시·구군 공공지원과 신청창구를 함께 확인하기 위한 것이며 실제 보험조건은 상품설명서와 약관을 기준으로 확인해야 합니다.'},{q:`${district.name} 출산지원과 태아보험은 함께 받을 수 있나요?`,a:'공공 출산지원과 민간보험은 서로 다른 제도입니다. 각각의 대상조건과 보험금 지급사유를 별도로 확인하며 공공지원 금액을 보험 보장금액처럼 합산하지 않습니다.'}];
 const canonical=`${site.baseUrl}/태아보험/부산태아보험/${district.slug}`;
 const schemas=[{"@context":"https://schema.org","@type":"WebPage",name:`${keyword} 상담 전 가이드`,description:district.intro,url:canonical,dateModified:site.contentReviewedAt,about:['태아보험','부산 출산지원',`${district.name} 출산·육아 지원`,...seoIntent.related],publisher:{"@type":"Organization",name:site.insuranceName,url:site.baseUrl}},{"@context":"https://schema.org","@type":"FAQPage",mainEntity:faq.map(x=>({"@type":"Question",name:x.q,acceptedAnswer:{"@type":"Answer",text:x.a}}))},{"@context":"https://schema.org","@type":"BreadcrumbList",itemListElement:[['홈',site.baseUrl],['태아보험',`${site.baseUrl}/태아보험`],['부산 태아보험',`${site.baseUrl}/태아보험/부산태아보험`],[keyword,canonical]].map(([name,item],i)=>({"@type":"ListItem",position:i+1,name,item}))}];
 return <main className="insurancePage">
  ...
  <section className="section editorialGuide"><div className="wrap editorialGuideGrid"><div><span className="insuranceBadge">이 페이지의 보험 주제</span><h2>{district.name}에서는 ‘{district.theme}’를 중심으로 봅니다</h2></div><div><p>지역명만 바뀐 복제 페이지가 되지 않도록 <b>{district.theme}</b>을 독립적인 보험 비교 소재로 사용합니다. 공공지원은 보험상품과 분리해 공식자료 기준으로 확인합니다.</p><p><b>함께 보는 검색 주제:</b> {seoIntent.related.join(' · ')}</p></div></div></section>
