...
export default function InsuranceInquiryForm({position,label}:{position:'primary'|'secondary';label:string}){
 ...
 return <section className={`inquirySection inquiry-${position}`} id={sectionId} aria-labelledby={`${sectionId}-title`} tabIndex={-1}>
  <div className="wrap inquiryWrap">
   ...
   <div className="inquiryFrameShell">
    <div className="inquiryFrameNotice" id={noticeId}><b>상담 신청 전 안내</b><span>입력한 정보는 상담 연결을 위해 제휴 상담처로 전달될 수 있습니다. 올바른 보험은 보험상품을 직접 판매하거나 가입을 확정하지 않습니다.</span></div>
    <iframe name="ifrm_icode" id={iframeId} scrolling="no" frameBorder="0" width="100%" src={FORM_SRC} title={`${label} 태아보험 상담 신청`} loading={position==='primary'?'eager':'lazy'} referrerPolicy="strict-origin-when-cross-origin" aria-describedby={`${noticeId} ${disclosureId}`} />
