...
export function isInsurancePath(pathname:string){
 ...
 return normalized==='/'||normalized.startsWith('/태아보험')||insuranceKeywordPaths.some(p=>normalized===p);
}
...
export function InsuranceHeader(){
 ...
 return <header className="header insuranceTopHeader">
  <div className="wrap insuranceNav">
   <Link href="/" className="insuranceLogo" aria-label="올바른 보험 홈" onClick={closeMobile} style={{display:'flex',alignItems:'center',justifyContent:'flex-start',flexShrink:0}}><img src="/images/insurance/olbareun-insurance-logo.webp" alt="올바른 보험" width="205" height="54" style={{display:'block',width:'clamp(155px,15vw,205px)',height:'auto',maxHeight:'54px',objectFit:'contain',objectPosition:'left center'}} /></Link>
   <nav className="insuranceNavLinks" aria-label="보험 주요 메뉴"><Link href="/">태아보험 <span>⌄</span></Link><Link href="/태아보험가입시기">가입시기</Link><Link href="/태아보험순위비교">비교가이드</Link><Link href="/임신초기증상">임신정보</Link><Link href="/임산부선물">출산준비</Link></nav>
  ...
  <div id="insurance-mobile-menu" className={`insuranceMobileMenu${mobileOpen?' isOpen':''}`}>
   <nav className="wrap" aria-label="모바일 보험 메뉴">
    <Link href="/" onClick={closeMobile}><b>태아보험</b><small>보험 메인 · 지역별 정보</small></Link>
