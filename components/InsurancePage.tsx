 ...
 return <main className="insurancePage">
  ...
  <section className="insuranceHero approvedHero"><div className="wrap approvedHeroGrid">
   <div className="approvedHeroCopy"><span className="insuranceBadge">올바른 보험</span><h1>{label?`${label} 태아보험 상담`:'태아보험 상담 전'}<br/><em>{label?'맞춤 정보를 확인하세요':'꼭 알아야 할 정보'}</em></h1><p>{lead}</p>
    ...
    <div className="heroActions"><a className="btn btnPrimary btnLift" href={site.cpaUrl}>무료 상담 알아보기 →</a><a className="btn btnGhost" href="#지원정보">출산지원 정보 보기 →</a></div>
   </div>
   <div className="approvedHeroPhoto" style={{backgroundImage:`linear-gradient(90deg,rgba(255,255,255,.05),rgba(255,255,255,.05)),url('${heroImage}')`}}><div className="photoFallback"><span>🤰</span><strong>엄마와 아이의 첫 준비</strong><small>올바른 보험</small></div></div>
   <aside className="approvedServiceRail"><div className="railIcon">👥</div><strong>9개</strong><b>생활서비스<br/>통합 브랜드</b><span>올바른 보험</span><span>올바른 회생</span><span>올바른 이사</span><span>올바른 청소</span><span>올바른 인터넷</span><span>올바른 렌트카</span><span>올바른 렌탈</span><span>올바른 수리</span></aside>
  </div></section>
