...
const sharedDescriptionFinalizer=seoVariants.includes('finalizeInsuranceSeoDescription')&&seoVariants.includes("'태아보험 상담 전 확인할 항목: $1.'")&&seoVariants.includes('description,');
const schemaDescriptionAligned=structured.includes('description:seo.description');
const routeDescriptionAligned=route.includes('description:seo.description')&&!route.includes('polishSeoDescription');
const canonicalHostStable=site.includes("baseUrl: process.env.NEXT_PUBLIC_SITE_URL || 'https://www.adlines.co.kr'");
...
const checks={
 ...
 h1SchemaAligned,
 sharedDescriptionFinalizer,
 schemaDescriptionAligned,
 routeDescriptionAligned,
 canonicalHostStable,
 linkRotation
};
...
console.log(`메타 ↔ Schema 설명 단일 소스: ${sharedDescriptionFinalizer&&schemaDescriptionAligned&&routeDescriptionAligned?'확인':'보완 필요'}`);
console.log(`canonical 기본 호스트: ${canonicalHostStable?'www.adlines.co.kr':'보완 필요'}`);
