...
const checks={
 ...
 finalCtaStage:page.includes('정보 확인 완료 · 상담 연결')&&page.includes("const finalCtaLabel=region?'확인한 내용으로 상담 신청 →':'무료 상담 알아보기 →'"),
...
console.log(`광역 slug 스캔: ${regionSlugs.length}개`);console.log(`키워드 페이지 slug 스캔: ${keywordSlugs.length}개`);console.log(`관련 가이드 링크: ${relatedHrefs.length}개`);console.log(`유효하지 않은 관련 링크: ${badRelated.length?badRelated.join(', '):'없음'}`);console.log(`지역 CTA 내부 신청서 연결: ${checks.regionalCtaRouting?'연결':'누락'}`);console.log(`FAQ 중복 제거·근거 우선순위: ${checks.faqDeduplication&&checks.faqEvidencePriority?'연결':'누락'}`);console.log(`하단 CTA 단계화: ${checks.finalCtaStage?'연결':'누락'}`);console.log(`지역 근거 상태 접근성: ${checks.supportContextA11y?'연결':'누락'}`);console.log(`지역 정책 최신성 UI: ${checks.freshnessIntegrated?'연결':'누락'}`);console.log(`지역 지원 읽기 가이드: ${checks.verifiedSupportReadingGuide?'연결':'누락'}`);console.log(`미확인 지역 신뢰 안내: ${checks.emptySupportTrust?'연결':'누락'}`);console.log(`출산 전후 일정표: ${checks.timelineIntegrated&&checks.timelineStages&&checks.timelineCss?'연결':'누락'}`);console.log(`상담 신뢰·모바일 전환: ${checks.inquiryDisclosure&&checks.inquiryPreparation&&checks.inquiryMobileJump&&checks.inquiryLazySecondary&&checks.inquiryExternalForm?'연결':'누락'}`);console.log(`정보 작성 원칙: ${checks.editorialStandards&&checks.editorialStandardsMounted?'연결':'누락'}`);console.log(`편집 기준일 중앙관리: ${checks.centralizedReviewDate?'연결':'누락'}`);console.log(`WebPage 최신성·발행주체 Schema: ${checks.schemaModified&&checks.schemaPublisher?'연결':'누락'}`);console.log(`구조화데이터/내부링크 검사: ${failed.length?failed.join(', '):'통과'}`);
if(badRelated.length||failed.length){process.exitCode=1;console.log('감사 결과: 보완 필요');}else console.log('감사 결과: 내부링크·CTA·FAQ 우선순위·Schema·정책표시·일정표·상담·작성원칙 핵심 항목 통과');
