...
console.log(`광역 slug 스캔: ${regionSlugs.length}개`);console.log(`키워드 페이지 slug 스캔: ${keywordSlugs.length}개`);console.log(`관련 가이드 링크: ${relatedHrefs.length}개`);console.log(`유효하지 않은 관련 링크: ${badRelated.length?badRelated.join(', '):'없음'}`);console.log(`지역 정책 최신성 UI: ${checks.freshnessIntegrated?'연결':'누락'}`);console.log(`출산 전후 일정표: ${checks.timelineIntegrated&&checks.timelineStages&&checks.timelineCss?'연결':'누락'}`);console.log(`상담 신뢰·모바일 전환: ${checks.inquiryDisclosure&&checks.inquiryPreparation&&checks.inquiryMobileJump&&checks.inquiryLazySecondary&&checks.inquiryExternalForm?'연결':'누락'}`);console.log(`정보 작성 원칙: ${checks.editorialStandards&&checks.editorialStandardsMounted?'연결':'누락'}`);console.log(`WebPage 최신성·발행주체 Schema: ${checks.schemaModified&&checks.schemaPublisher?'연결':'누락'}`);console.log(`구조화데이터/내부링크 검사: ${failed.length?failed.join(', '):'통과'}`);
if(badRelated.length||failed.length){process.exitCode=1;console.log('감사 결과: 보완 필요');}else console.log('감사 결과: 내부링크·Schema·정책표시·일정표·상담·작성원칙 핵심 항목 통과');
