import fs from 'node:fs';
import path from 'node:path';
const root=process.cwd();const read=p=>fs.readFileSync(path.join(root,p),'utf8');
const data=read('lib/insurance-data.ts');
const supportFiles=['lib/regional-support.ts','lib/city-support.ts','lib/city-support-central.ts','lib/city-support-south.ts','lib/city-support-east.ts','lib/city-support-gyeongnam.ts','lib/city-support-jeju.ts','lib/city-support-gangwon.ts'].map(read).join('\n');
const page=read('components/InsurancePage.tsx');
const context=read('lib/support-context.ts');
const citySlugs=[...data.matchAll(/slug:\s*['"]([^'"]+태아보험)['"]/g)].map(m=>m[1]);
const unique=[...new Set(citySlugs)];
const regionSlugs=new Set(['서울태아보험','부산태아보험','대구태아보험','인천태아보험','광주태아보험','대전태아보험','울산태아보험','세종태아보험','경기태아보험','강원태아보험','충북태아보험','충남태아보험','전북태아보험','전남태아보험','경북태아보험','경남태아보험','제주태아보험']);
const cities=unique.filter(s=>!regionSlugs.has(s));
const missing=cities.filter(s=>!supportFiles.includes(s));
const scopeChecks={
 cityEvidenceFlag:page.includes('hasVerifiedCitySupport'),
 regionalFallbackLabel:page.includes('supportLabel'),
 cityEmptyNotice:page.includes('<RegionalSupportEmpty label={city.name}/>'),
 timelineScope:page.includes('timelineSupportCount'),
 nonCashContext:context.includes('hasNonCashSupport')&&context.includes('지급방식 함께 확인')&&context.includes('현금성 출산지원금으로 해석하지 말고'),
 nonCashFaq:context.includes('모두 현금으로 받는 출산지원금인가요?')&&context.includes('바우처, 서비스 이용, 지역화폐')
};
const failedScopeChecks=Object.entries(scopeChecks).filter(([,ok])=>!ok).map(([key])=>key);
console.log('\n=== 도시 지원정보 커버리지 ===');
console.log(`도시 페이지: ${cities.length}개`);console.log(`별도 지원데이터 미확인: ${missing.length}개`);console.log(missing.length?missing.join(', '):'모든 도시 지원데이터 연결');
console.log(`도시/광역 지원 구분: ${failedScopeChecks.length?'보완 필요 '+failedScopeChecks.join(', '):'통과'}`);
console.log(`비현금성 지원 오해 방지: ${scopeChecks.nonCashContext&&scopeChecks.nonCashFaq?'통과':'보완 필요'}`);
console.log('※ 별도 도시자료가 미확인인 페이지는 상위 광역 지원을 도시 자체 지원처럼 표시하지 않고, 도시 자체사업 최신 공고 재확인 안내를 별도로 노출합니다.');
console.log('※ 바우처·서비스·지역화폐·물품형 지원은 현금성 출산지원금으로 단정하지 않고 지급형태와 사용조건을 별도로 안내합니다.');
if(failedScopeChecks.length)process.exitCode=1;
