...
const files={
  ...
  variants:read('lib/insurance-page-variants.ts'),
...
const missingEditorial=citySlugs.filter(s=>!files.content.includes(`/${s}'`)&&!files.content.includes(`/${s}\"`)&&!files.content.includes(`/${s}:`));
...
const requiredPageTokens=['getLocalEditorial','getInsuranceSeo','getContextualFaq','getRelatedLinks','getChecklistDetail','<InsuranceInquiryForm','getInsuranceImages','relatedGuideSection'];
...
const imageChecks={
  webp:files.images.includes(".webp`"),
  fiveSlots:files.images.includes("padStart(2,'0')")&&files.images.includes('hubKeywords'),
  localSceneAlt:files.images.includes('sceneSets')&&files.images.includes('scenes[index]'),
  regionFolder:files.images.includes('${region.slug}/${city.slug}')
};
const failedImageChecks=Object.entries(imageChecks).filter(([,v])=>!v).map(([k])=>k);

const seoChecks={
  localizedTitle:files.content.includes('태아보험 상담 | 가입시기·출산지원·보장 비교'),
  localizedDescription:files.content.includes('2026 지역 출산·육아 지원'),
  localizedH1:files.content.includes('태아보험 상담`'),
  baseFaq:files.content.includes('export function getInsuranceFaq'),
  contextualFaq:files.variants.includes('export function getContextualFaq'),
  relatedLinks:files.variants.includes('export function getRelatedLinks'),
  regionAwareEditorial:files.content.includes('CITY_PROFILES')&&files.content.includes('REGION_PROFILES')&&files.content.includes('cityKey')
};
...
const collisionHandled=!knownCollision||files.content.includes("'경기태아보험/광주태아보험'");
...
if(fs.existsSync(publicInsuranceDir)){
...
if(contextualFaqCount<5)errors.push('지역별 FAQ 변형 풀 부족');
...
console.log(`도시 지원 데이터 미검증/누락: ${missingSupport.length}개${missingSupport.length?' -> '+missingSupport.join(', '):''}`);
console.log(`도시 고유 편집문구 누락: ${missingEditorial.length}개${missingEditorial.length?' -> '+missingEditorial.join(', '):''}`);
console.log(`광역 고유 편집문구 누락: ${missingRegionEditorial.length}개`);
console.log(`상담폼 배치 신호: ${inquiryCount}개`);
console.log(`지역 FAQ 변형 풀: ${contextualFaqCount}개`);
console.log(`관련 가이드 링크 풀: ${keywordLinkCount}개`);
console.log(`이미지 규칙: ${failedImageChecks.length?'보완 '+failedImageChecks.join(', '):'통과'}`);
...
console.log(`실제 보험 WebP: ${actualWebpCount}개${actualWebpCount===0?' (바이너리 업로드 별도 필요)':''}`);
...
if(errors.length){
  ...
  console.log('\n감사 결과: 핵심 SEO·지역화·FAQ·내부링크·상담 구조 통과');
  if(missingSupport.length)console.log('주의: 공식 최신 지원금이 충분히 검증되지 않은 도시는 금액을 추정하지 않고 일반 지역정보만 노출합니다.');
  if(actualWebpCount===0)console.log('주의: 이미지 경로/ALT 규칙은 준비됐지만 실제 WebP 바이너리 업로드는 별도 단계입니다.');
}
