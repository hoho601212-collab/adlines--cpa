...
const rows=blocks.map(block=>({block,title:pick(block,'title'),sourceName:pick(block,'sourceName'),url:pick(block,'sourceUrl'),verifiedAt:pick(block,'verifiedAt')}));
const now=new Date();
...
if(invalid.length||legislativeUnclear.length){console.log('감사 결과: 근거 표현 보완 필요');process.exitCode=1;}else console.log('감사 결과: 기본 형식 통과 · 조례/계획 근거는 시행상태를 별도 표시');
