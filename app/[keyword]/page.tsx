...
export async function generateMetadata({params}:Props):Promise<Metadata>{
 const raw=await params;const keyword=normalizeKeyword(raw.keyword);const page=getKeywordPage(keyword);if(!page)return{};
...
export default async function Page({params}:Props){const raw=await params;const keyword=normalizeKeyword(raw.keyword);const page=getKeywordPage(keyword);if(!page||page.slug==='태아보험')return notFound();return <KeywordGuidePage page={page}/>}
