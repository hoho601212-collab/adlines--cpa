...
export default function sitemap():MetadataRoute.Sitemap{
 const now=new Date();return[
  ...
  ...regions.flatMap(r=>[{url:`${site.baseUrl}/태아보험/${r.slug}`,lastModified:now,priority:.85},...r.cities.map(c=>({url:`${site.baseUrl}/태아보험/${r.slug}/${c.slug}`,lastModified:now,priority:.75}))]),
  ...busanDistricts.map(d=>({url:`${site.baseUrl}/태아보험/부산태아보험/${d.slug}`,lastModified:now,priority:.8}))
 ];
}