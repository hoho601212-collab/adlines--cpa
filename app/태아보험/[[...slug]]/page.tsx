...
export async function generateMetadata({params}:Props):Promise<Metadata>{
 ...
 if(r.slug==='부산태아보험'&&slug[1]){const d=findBusanDistrict(slug[1]);if(!d)return{};const title=`${d.name} 태아보험 상담 | 가입시기·보장·부산 출산지원`;const description=`${d.name} 태아보험 상담 전 ${d.theme}을 중심으로 가입시기와 보장조건을 확인하고, 2026 부산시 공통지원과 ${d.name} 출산·육아 체크사항을 구분해 살펴보세요.`;const canonical=`/태아보험/부산태아보험/${d.slug}`;return{title,description,alternates:{canonical},robots,openGraph:{title,description,url:canonical,type:'website'},twitter:{card:'summary_large_image',title,description}};}
