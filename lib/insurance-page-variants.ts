...
const faqPools=[
 ...
 (label:string):FaqItem=>({question:`${label}에서 다태아 출산이면 지원 기준이 달라질 수 있나요?`,answer:`가능합니다. 일부 사업은 출생아 수를 기준으로 각각 지원하고, 일부는 출생순위나 가구 기준을 적용합니다. 쌍둥이·다태아라면 ${label} 공고에서 다태아 산정방식과 신청서류를 별도로 확인하세요.`}),
 (label:string):FaqItem=>({question:`${label} 지원은 현금으로만 지급되나요?`,answer:`그렇지 않습니다. 현금, 지역화폐, 바우처, 서비스 이용권, 물품지원처럼 지급방식이 다양합니다. 총액만 보지 말고 사용처·유효기간·분할지급 여부를 함께 확인해야 실제 활용 가능성을 판단할 수 있습니다.`}),
 (label:string):FaqItem=>({question:`${label} 출산지원과 첫만남이용권은 같은 제도인가요?`,answer:`아닙니다. 첫만남이용권은 전국 공통제도이고, ${label} 출산축하금·장려금 등은 지자체 자체사업일 수 있습니다. 신청창구와 자격요건이 다를 수 있으므로 두 제도를 별도로 확인하세요.`}),
 (label:string):FaqItem=>({question:`${label} 태아보험 가입시기와 지역지원 신청시기를 같이 봐야 하나요?`,answer:`일정을 함께 정리하면 편하지만 두 제도의 기준은 다릅니다. 보험은 가입 가능시기·심사·보장개시 조건을, 지역지원은 임신주수·출생신고·거주기간·신청마감일을 각각 기준으로 확인하세요.`}),
...
function checklistMessage(item:string,place:string,index:number){
 if(/거주|주소|전입|주민등록/.test(item))return `📍 ${place} 지원은 부모·아동의 주민등록 주소와 계속 거주기간을 기준일별로 확인하세요. 출생일·신청일·후속 지급일의 주소요건이 서로 다를 수 있습니다.`;
 if(/신청|기한|기간|시기/.test(item))return `🗓️ 신청 가능한 시점을 달력에 따로 표시하세요. 임신 중 신청, 출생신고 직후 신청, 생후 일정 기간 안에 신청하는 사업을 구분하면 누락을 줄일 수 있습니다.`;
 if(/지원|수당|장려|축하|바우처|교통|주거|산후/.test(item))return `🎁 지원액만 보지 말고 대상, 지급수단, 분할지급 여부와 사용기한까지 함께 확인하세요. ${place} 자체사업과 전국 공통제도는 별도로 신청되는 경우가 있습니다.`;
 if(/가입|보험|보장|특약|갱신/.test(item))return `🛡️ 보험은 상품설명서와 약관을 기준으로 가입 가능시기, 보장기간, 면책·감액 조건, 갱신 여부를 확인하고 지역지원과는 별도로 판단하세요.`;
 if(/출생|출산|다자녀|출생순위/.test(item))return `👨‍👩‍👧 출생순위와 다태아 산정기준은 사업마다 다릅니다. ${place} 공식 공고에서 첫째·둘째 이상 기준과 후속 지급 조건을 함께 확인하세요.`;
 const fallback=[`🔎 ${place}에서 실제 적용되는 조건을 확인하려면 최신 공식 공고의 대상·기준일·신청창구를 함께 보세요.`,`📝 상담 전에 필요한 정보와 지역지원 신청정보를 따로 메모해 두면 보험 조건과 공공지원이 섞이지 않아 비교가 쉬워집니다.`,`✅ 확인한 내용은 출처와 확인일을 함께 적어 두고 실제 신청·가입 직전에 다시 확인하세요.`];
 return fallback[index%fallback.length];
}

export function getChecklistDetail(item:string,index:number,label?:string){
 const place=label||'해당 지역';
 return `${item}: ${checklistMessage(item,place,index)}`;
}
