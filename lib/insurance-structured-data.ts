...
import {getLocalizedInsuranceSeo} from './insurance-seo-variants';
...
export function getInsuranceStructuredData({region,city,faqs,relatedLinks}:{region?:Region;city?:City;faqs:FaqItem[];relatedLinks:RelatedLink[]}){
  ...
  return {
    webpage:{'@context':'https://schema.org','@type':'WebPage','@id':`${url}#webpage`,url,name:`${seo.h1} ${seo.h1Accent}`,description:seo.description,isPartOf:{'@id':`${site.baseUrl}#website`},inLanguage:'ko-KR',dateModified:site.contentReviewedAt,about:[{'@type':'Thing',name:'태아보험'},{'@type':'Thing',name:'출산·육아 지원정보'}],publisher:{'@type':'Organization',name:'올바른 보험',url:site.baseUrl}},
